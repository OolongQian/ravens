...
class Dataset:
    ...
    def load(self, episode_id, images=True, cache=False):
        ...
        for fname in sorted(tf.io.gfile.listdir(path)):
            if f'{episode_id:06d}' in fname:
                ...
                # # do: check whether we have the desired goal conditioned situation.
                # import cv2
                # for i in range(3):
                #     rgb = color[i, 0, ...]
                #     cv2.imshow('haha', rgb)
                #     cv2.waitKey(0)
                # exit(0)
                # do: there are three images from multiple angles per instance.
                # import cv2
                # cv2.imshow('haha', color[0, 2, ...])
                # cv2.waitKey(0)
                # print(color)
                # exit(0)
    ...
    def sample(self, images=True, cache=False):
        ...
        # say: episode ending would never be chosen as training samples.
